...
[纵向生命周期](03_vertical_lifecycle_walkthrough.md)沿一次配置修复任务解释了 Harness 在什么时候建立会话、准备本轮上下文、调用模型、执行工具、压缩历史或恢复任务。本章把这条时间线折叠成一张结构图：哪些部分负责作决定，哪些部分真正改变工作区，哪些对象承载会话连续性，哪些结果可以交给用户检查。
...
图 4-1 先给出全貌。一次请求从客户端进入会话，由 Harness 循环组织下一步；上下文构造器选择模型本次可见的内容，模型服务路由器负责连接具体模型服务。模型若提出工具动作，工具适配层把它解析成参数明确的调用，能力目录则决定模型能够提出哪些动作；权限策略决定是否放行，执行器再在沙箱、容器或宿主限制下访问工作区。执行结果以事件和环境观察回流，同时形成可以保存或交付的任务产物（Artifact）。
...
flowchart TB
  ...
  subgraph CP[控制平面]
    S[任务身份与连续性<br/>会话（Session）/ 轮次（Turn）]
    O[编排与终止判断<br/>运行循环（Harness Loop）]
    X[输入投影与模型选路<br/>上下文构造器（Context Builder）/ 模型服务路由器（Provider Router）]
    T[能力目录<br/>工具描述（Tool Schema）/ 能力目录（Capability Catalog）]
    P[策略与授权<br/>独立 Policy 可合并或由宿主外置]
  end

  subgraph DP[执行平面]
    G[工具注册与适配<br/>注册表（Registry）/ 适配器（Adapter）]
    E[动作执行器（Executor）]
    B[实际限制<br/>宿主策略；可选沙箱（Sandbox）/ 容器（Container）]
  end

  subgraph ST[状态与产物]
    V[状态变化与记录<br/>事件（Event）/ 持久化（Persistence）]
    R[可选的跨调用取回信息<br/>可检索记忆（Memory）]
    A[可检查结果<br/>任务产物（Artifact）]
  end
...
*图 4-1　概念图：Agent Harness 统一参考架构。控制平面组织任务和授权，执行平面改变工作区或外部资源，状态与产物沿结果路径回流；责任角色可以合并、缺席或由宿主外置，不表示七个固定版本都具有同名组件或全部转换。*

控制平面与执行平面是责任划分，不一定是两个进程。单进程命令行客户端可以同时包含输入、模型循环、工具执行和终端输出；服务化产品则可能让客户端只负责提交请求和显示事件，由后台服务持有会话，再把工具交给容器或协议服务器执行。前者部署简单，但共享同一权限与故障域；后者更容易隔离和支持多客户端，却要额外处理身份、断线、取消和状态同步。
...
工具描述（Tool Schema）、授权和执行限制也要分开理解。工具描述划定模型可以提出的动作表面，权限策略对参数明确的单次调用作出决定，执行环境则给真实副作用设定资源上限。模型能看见 `bash` 能力，只表示它可以向命令行解释器（Bash）提出命令；从提议到副作用仍须经过[纵向生命周期所述的授权与受限执行](03_vertical_lifecycle_walkthrough.md#从行动提议到真实副作用)。
...
| 对象 | 含义 | 在配置修复任务中的例子 |
| --- | --- | --- |
| **会话（Session）** | 围绕一项可连续或恢复任务的逻辑容器，关联身份、历史与控制状态 | “修复配置解析错误”这项任务及其多次交互 |
| **轮次（Turn）** | 从一次新输入或内部续行开始，到完成、失败、取消、等待或交还控制为止的处理区间 | 用户发出请求后，模型经历多次读写与测试，最终返回说明 |
| **消息（Message）** | 具有语义角色的内容，如用户输入、模型回复或工具结果 | 用户要求、模型解释、测试失败文本 |
| **事件（Event）** | 表示状态随时间发生变化的通知或记录 | 工具进入执行、审批被拒绝、测试结束 |
| **工作单元（Item）** | 可排序、引用或展示的工作单元，可容纳消息、工具调用、结果、审批与摘要 | 一次文件读取调用及其结果 |
| **本轮上下文（Context）** | 某一次模型调用实际可见的输入投影 | 项目指令、相关历史、文件片段和工具说明 |
| **可检索记忆（Memory）** | 位于当前调用之外、可保存并在以后检索或注入的信息源 | 可跨任务取回的项目约定或用户偏好 |
| **任务产物（Artifact）** | 任务产生、可被外部检查或后续消费的有界结果 | diff、修改后的文件、带退出状态的测试输出、最终报告 |

*表 4-1　Agent Harness 的八个核心对象及其在配置修复任务中的对应关系。*

这些对象构成了任务的基本关系。一个 Session 可以包含多个 Turn；一个 Turn 可以读取旧 Item，并产生新的 Message、Tool Call、工具结果（Tool Result）、审批或摘要 Item。Message 强调“内容扮演什么角色”，Event 强调“状态发生了什么变化”，因此同一个测试结果既可以作为工具结果进入模型历史，也可以触发“测试已完成”的 Event。Artifact 面向任务外部：用户可以检查 diff 和测试输出，而内部调度器的等待状态通常只是 Event，不是交付物。

Turn 的边界由控制权变化决定，不必等于一次模型请求。模型可以在同一 Turn 内多次调用工具、根据失败结果再次采样，也可以等待用户批准后继续。若系统把任务交还给用户，用户补充条件后再开始，通常更适合视为新的 Turn。这样的定义适用于集中式编辑循环，也适用于事件驱动会话，不会强迫七个系统共享一种内部表示。
...
Compaction 是有损变换，至少要保住几项不变量：用户目标和禁止事项仍可识别，未完成调用不会被总结为已完成，最近的失败与修改位置仍可追踪，Session 身份与分支关系不会因摘要而改变。令牌节省因此不是简单删除最旧消息，而是保护约束和未决状态的选择问题；摘要若模糊了失败或未决状态，下一轮就可能错误结束任务。

## 能力、协议与客户端：同一工具为什么会有不同体验

能力回答“系统能做什么”，协议回答“组件怎样表达请求和结果”，客户端回答“谁怎样使用这些能力”。文件读取、补丁编辑、Shell、Git、搜索和测试属于能力；MCP、Agent 客户端协议（Agent Client Protocol，ACP）、应用内消息或模型接口可以承载这些能力；命令行界面（Command-Line Interface，CLI）、终端用户界面（Terminal User Interface，TUI）、集成开发环境（Integrated Development Environment，IDE）、网页界面（Web）与软件开发工具包（Software Development Kit，SDK）则负责选择入口、展示进度和收集用户决定。这里的 CLI 是通用客户端类别，不是 Gemini CLI 产品名的简称。

三层可以重合，却不能互相代替。某个系统提供 MCP 服务，并不表示默认命令行界面已经启用该服务；终端用户界面能显示审批，并不保证无界面的 SDK 会采用同样交互；Shell 能运行 Git，也不表示 Harness 实现了 Git 事务。多入口产品尤其需要说明哪些入口共享 Session 和执行策略，哪些入口只有相同的模型调用，却采用不同的工具集或权限方式。

SWE-agent 对智能体—计算机接口（Agent–Computer Interface，ACI）的研究提供了一个实用视角：能力的质量不只由工具名称决定，还取决于参数是否简洁、工作目录是否明确、错误是否完整、输出是否标记截断、调用与结果是否能准确配对 [@yang2024sweagent]。这也解释了协议适配器的真正职责。适配器不能只转发一段数据，还要保留流式增量、调用标识、取消、结束原因、并发顺序和错误语义；若把拒绝包装成成功文本，下一层就会对现实状态产生错误判断。

扩展同时改变能力和信任。进程内插件（Plugin）或扩展（Extension）可以注册工具、改写 Context、拦截调用，并继承宿主进程权限；通过协议接入的外部工具改善语言和进程解耦，却引入服务器身份、凭据、网络和描述漂移问题。无论能力从哪里接入，“模型看得见工具”“动作得到授权”“执行器可以访问资源”仍然是三个需要分别回答的问题。
...
Saltzer 与 Schroeder 提出的完全仲裁与最小权限原则，分别要求每次访问都经过检查，并只授予完成任务所需的最小权力 [@saltzer1975protection]。放到 Harness 中，工具注册时检查一次并不够，批准之后的扩展也不应悄悄扩大路径或网络目标。参考架构因此分别标出人类意图、运行时授权与执行环境限制，并把纵向生命周期已经说明的控制关系落实到不同组件责任上。
...
这条边界也决定完成条件。diff、带退出状态的测试记录和最终解释都可以成为 Artifact，分别保留修改、验证与交付说明的可检查证据；会话中保存一句“测试通过”只是一条历史内容，当前是否通过仍取决于新鲜的执行结果。
