...
Aider 的官方工程长文记录了这条设计的早期成形：tree-sitter 提取定义与引用，文件关系图参与排序，再由 `map_tokens` 预算选择最相关的符号签名 [@gauthier2023aiderepomap]。文章在 2023 年仍把自动寻找可编辑文件列为后续方向，因此它支撑 Repo Map 的结构选择，不应被用来声称当时已经自动完成文件准入。
...
Unified Diff 路径的形成也有公开的工程实验背景。Aider 作者在 2023 年针对 `gpt-4-1106-preview` 比较编辑格式，并把熟悉、简单、高层和可容错作为设计原则 [@gauthier2023aiderunifieddiffs]；其中的成功率与错误倍数只适用于当时模型和 Aider 自有任务，不能外推为 2026 年模型的普遍结论。
...
官方 Architect/Editor 长文把这一机制解释为“推理解法”与“遵循编辑协议”的两阶段分工，并用 Aider 自有编辑基准报告当时的组合结果 [@gauthier2024aiderarchitect]。这些数字不是 SWE-Bench，文章描述的实验支持也不能被升格为默认路径或通用多 Agent 编排。
...
Aider 的官方工程长文可以按三项代表性机制继续阅读：Repo Map 文章解释 tree-sitter、图排序与 Token 预算 [@gauthier2023aiderepomap]，Architect/Editor 文章解释推理与编辑协议分工 [@gauthier2024aiderarchitect]，Unified Diff 文章记录特定旧模型上的格式实验 [@gauthier2023aiderunifieddiffs]。三者均为创始人博客而非论文，基准数字必须保留模型代际、任务集和日期边界。
