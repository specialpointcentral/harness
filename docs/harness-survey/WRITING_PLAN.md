...
- 当前状态：已确认总体方向，等待书面复核
- 最近修订：吸收书面评审，补充叙事式 Index、篇幅与发布预算、读者画像路线、遥测隐私、
  接口形态对比、分析环境记录和 Submodule preflight；保留代码先行、文献检索与克制
  限定要求
...
| 读者 | 建议起点和主线 | 可以稍后阅读 |
|---|---|---|
| 第一次接触 Harness 的读者 | `00` → `03` → `04` → `05` → `07` → `08` → `12` → 任选一个个案章 → `30` | `01` 中的方法细节、完整证据索引和研究议程 |
...
| 安全研究者 | `01` 的证据方法 → `04` → `07`–`10` → `12` → `16`–`17` → `19` → `22` → `30`–`32` | UI 细节和非相关项目实现 |

初学者路线先建立“一个任务如何流经 Harness”的直觉，不要求先掌握证据等级、
有效性威胁和完整比较方法。方法章仍然保留为研究透明度入口，并在读者需要判断
证据强度时回读。
...
计划目录为 `docs/harness-survey/`。正文和附录共规划 37 个编号 Markdown 文件。
...
#### `01_scope_and_methodology.md`：范围、术语与研究方法
...
#### `92_evidence_index.md`：证据索引

- 按章节索引源码证据
- 按 Harness 索引源码证据
- 结论、调用链、路径、symbol 和证据状态的映射
- 七系统 Tool-call 规范化 schema 与真实字段的映射
- 官方文档证据
- 运行验证证据
- 尚未验证的推断
- 证据更新记录
...
| Tool-call 接口形态与字段映射 | `06_model_and_provider_abstraction.md`、`08_tool_call_system.md`、`92_evidence_index.md` |
...
不得把 `Not identified` 写成“不支持”。如果某能力只出现在测试、实验目录或
未接入默认产品路径，应标记为 `Experimental` 或 `Not central` 并解释。
...
不足以形成若干连贯段落，就应合并。不得省略“比较、权衡、证据”三个要素。
...
篇幅以中文正文字符估算，不含代码、表格、图注、参考文献和证据索引。初始目标为：

| 章型 | 单章初始目标 | 说明 |
|---|---:|---|
| `00` 叙事式序章 | 4,000–6,000 字 | 以完整故事建立概念，并自然引出阅读入口 |
| `01`–`04` 方法和总览 | 3,000–6,000 字 | 建立方法、比较视角和统一模型 |
...
描述，是否应把源码地图移入 `92_evidence_index.md`，以及某个项目特性是否更适合
...
2. **最小可发布核心版**：完成 `00`–`18`、七个个案章 `23`–`29`、综合比较
   `30` 和附录 `90`–`93`。它形成从参考架构、核心 Runtime 到 Coding 工作流的
...
> **实现证据**
>
> - 项目、commit、源码路径和关键 symbol；
> - 官方文档或测试；
> - 必要时给出运行验证状态。
```

未来 Pandoc 模板可根据加粗标签把五类 blockquote 渲染为不同颜色的
...
- 不用当前行号作为唯一引用，至少同时记录路径和 symbol；
- 引用仓库 URL 时固定到 commit，而不是浮动的默认分支。
...
正文不必给每句话添加标签，但能力矩阵、争议结论和安全判断应在证据索引中
可追溯。
...
源码片段。详细路径、symbol、commit、测试和证据状态进入实现证据框及
`92_evidence_index.md`。
...
正文使用台账能够支持的表述强度。代码与官方文档不一致时，将差异作为调查结果，
...
机制落地，最后回到真实 Harness 的实现证据；也可以在章首先提出问题场景，随后
逐步抽象，但不能只讲故事而不总结机制。
...
教学案例必须明确标为示例，不得冒充七个项目的运行证据。必要的伪代码、短对话、
状态快照和时序图应尽量围绕同一案例展开，避免每节重新发明无关场景。示例结束后
必须指出它对应的真实实现入口、可泛化部分和不能泛化的部分。
...
不能未经抽象直接进入正文；应先说明该细节证明了哪个机制结论，再把路径和 symbol
放入实现证据框或证据索引。

### 16.3 克制使用限定和否定

这是一份源码驱动的工程教科书式报告，不采用论文答辩或审稿回复式语气。正文默认
正面回答“这个机制做了什么、如何工作、产生什么效果”，随后给出代码证据和必要
例子。方法章已经统一说明范围、非目标和证据等级，各章不再反复声明“本文没有做
什么”“这不代表什么”或“我们不讨论什么”。
...
- 使用 Lua filter 把五类语义框转换为 HTML/PDF callout；
...
- 至少讨论一种重要设计取舍；
- 如使用特色机制框，已建立共同比较基线，并解释差异、动机、代价和证据状态；
- 安全结论说明攻击前提和证据状态；
- 没有不必要的大段源码；
...
- 横向地图中的每项能力都有详细章节；
- 纵向生命周期中的每个阶段都有落点；
- `00_index.md` 同时提供内容视角和四类读者路线，初学者无需先读完整方法章；
...
- 抽查的重要工程结论能够从正文回到固定 commit 的入口、调用链和测试；
- 没有把功能存在等同于安全保证；
