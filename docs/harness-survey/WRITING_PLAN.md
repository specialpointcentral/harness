...
- 当前状态（2026-09-11）：36 个编号章节均已存在，PDF 构建流程已验证；当前工作为修订维护
...
- 源稿格式：可移植 Markdown
- 输出形式：GitHub 可直接阅读的章节集合，以及由 Pandoc/XeLaTeX 汇编的 PDF
- 分析对象：Codex、OpenCode、Pi、Gemini CLI、DeepSeek Harness、Goose、Aider
- 基准父仓库提交：`b964dd3896239ff06e13c9efd363266755e5d9af`

这份文件保留详细编辑规范与早期章节设计，不纳入最终正文编号。阶段计划、篇幅预算和
候选标题属于设计记录，不表示当前待办或完成状态。贡献流程以
[贡献指南](../../CONTRIBUTING.md)为准，PDF 操作以[构建文档](../book/README.md)为准。
最终报告以 `00_index.md` 为序章、目录和阅读入口。
...
横向/纵向/个案三种内容视角和四类读者路线，最后给出与当前完成进度一致的目录。
写作中间状态下，`00_index.md` 只列出已经完成并可供阅读的章节，随后按章节组逐步
展开；全部章节到位后再补成完整目录。这样第一次阅读按叙事建立整体直觉，后续回查
仍可直接使用目录，也不会把尚未完成的章节误写成可阅读正文。
...
首次调整记录（2026-08-25）：`04_reference_architecture.md` 需要用八个核心对象和
两张图建立后续全书共用的术语与结构基线，正文预算由 `01`–`04` 的通用上限上调至
约 7,500 字。该调整仍在允许的约 25% 范围内，不要求删减现有正文，也不连带改变
`01`–`03` 的预算。
...
- 2026-08-25 第一部分读者 Review 保留了第 4.1 节第 1 项边界目标，并选择在
  `01_introducing_agent_harness.md` 补充相邻系统的朴素判定方法，而不是缩小目标。
  原因是 framework、MCP server、IDE agent、云端 control plane 与 benchmark
  harness 的边界会直接影响安全研究者和平台开发者判断资产、责任与信任边界。

## 23. 下一步

叙事序章与第一部分 `00`–`04` 已完成读者版正文并进入修订维护。下一步按阶段 4 的
知识依赖顺序分组撰写机制章：每组先完成固定版本源码追踪与必要文献核验，再直接
交付可连续阅读的完整正文、同步内部证据台账，并运行术语、引用键、相对链接、标题
层级、图表编号和语义框检查。`00_index.md` 随已完成章节组逐步展开，待全部章节到位
后补成完整目录。
